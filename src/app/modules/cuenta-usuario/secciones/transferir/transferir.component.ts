...
export class TransferirComponent {
  ...
  checkInputs():boolean{
    ...
    if(this.cuenta_destinataria.length <19 ){
      pasa=false
      this.util.enviarAlerta('warning',this.util.colorWarning,"Error en el formulario", "Formato en el campo cuenta destinataria invalido")
    }else if(this.monto <0){
      pasa=false
      this.util.enviarAlerta('warning',this.util.colorWarning,"Error en el formulario", "El monto no puede ser negativo")
    }else if(this.concepto.length <0){
      pasa=false
      this.util.enviarAlerta('warning',this.util.colorWarning,"Error en el formulario", "Formato en el campo direccion concepto no puede ser vacio")
    }else if(this.descripcion.length<0){
      pasa=false
      this.util.enviarAlerta('warning',this.util.colorWarning,"Error en el formulario", "Formato en el campo direccion descripcion no puede ser vacio")
    }
        ...
        next: (e) => {
            ...
            .subscribe({
              next: (e) => {
                ...
                this.usuario.usuarioss[0].cuenta.numeroCuenta = resultado.join(' ');
